        ...
        try:
            if config.type == McpServerType.STDIO:
                ...
                try:
                    ...
                    # 현재 환경 변수에 config.env 추가
                    env = os.environ.copy()
                    if config.env:
                        env.update(config.env)
                        print(f"🔧 서버 '{config.name}' 환경 변수 설정: {config.env}")
                        # GODOT_PATH가 실제로 설정되었는지 확인
                        # if 'GODOT_PATH' in env:
                        #     print(f"✅ GODOT_PATH 설정됨: {env['GODOT_PATH']}")
                    ...
                    # 응답 대기 - JSON-RPC 응답이 나올 때까지 여러 줄 읽기
                    response = None
                    max_attempts = 10  # 최대 10줄까지 시도
                    ...
                    for attempt in range(max_attempts):
                        ...
                        # JSON-RPC 응답인지 확인 ({"jsonrpc": "2.0"로 시작하는지)
                        try:
                            parsed_response = json.loads(response_line.strip())
                            if isinstance(parsed_response, dict) and "jsonrpc" in parsed_response:
                                response = parsed_response
                                break
                        except json.JSONDecodeError:
                            # JSON이 아닌 로그 메시지인 경우 - 다음 줄 읽기
                            print(f"📝 서버 '{config.name}' 로그: {response_line.strip()}")
                            continue
                    
                    if response is None:
                        raise Exception("JSON-RPC 초기화 응답을 찾을 수 없음")
...
def load_mcp_config(config_file: str = "test.json") -> List[McpServerConfig]:
    ...
    if not os.path.exists(config_file):
        print(f"❌ MCP 설정 파일을 찾을 수 없습니다: {config_file}")
        return []
    ...
    try:
        ...
        for server_name, server_config in config_data.get("mcpServers", {}).items():
            # command에서 args를 분리하여 처리 (playwright 등의 경우)
            command = server_config.get("command")
            args = server_config.get("args", [])
            
            # command가 "npx @playwright/mcp@latest" 형태인 경우 분리
        ...
        print(f"✅ {len(mcp_servers)}개의 MCP 서버 설정을 로드했습니다: {[s.name for s in mcp_servers]}")
        return mcp_servers
        
    except Exception as e:
        print(f"❌ MCP 설정 파일 읽기 실패: {e}")
        return []
...
async def main():
    ...
    # test.json 파일에서 MCP 서버 설정 로드
    mcp_servers = load_mcp_config("test.json")
    
    # 기본 예제 서버 추가 (test.json에 서버가 없는 경우)
    if not mcp_servers:
        print("⚠️  test.json에서 MCP 서버를 찾을 수 없어 기본 예제 서버를 사용합니다.")
        mcp_servers = [
            McpServerConfig(
                name="example-server",
                type=McpServerType.STDIO,
...
# load_mcp_config 함수는 위에서 이미 정의되었습니다.
    ...
    if not mcp_servers:
        ...
        mcp_servers = [
            McpServerConfig(
                ...
                type=McpServerType.STDIO,
                command="a:\\mcpclient\\venv\\Scripts\\python.exe",
                args=["a:\\mcpclient\\example_mcp_server.py"],
                timeout=10
            ),
        ]
